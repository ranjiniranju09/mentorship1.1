    ...
    <div class="card-body">
        <form method="POST" action="{{ route('admin.guest-lectures.update', [$guestLecture->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                {{--<input class="form-control {{ $errors->has('guessionsession_title') ? 'is-invalid' : '' }}" type="text" name="guessionsession_title" id="guessionsession_title" value="{{ old('guessionsession_title', $guestLecture->guessionsession_title) }}" required>--}}
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('speaker_name') ? 'is-invalid' : '' }}" name="speaker_name_id" id="speaker_name_id">
                    ...
                    <option value="{{ $id }}" {{ old('speaker_name_id', $guestLecture->speaker_name_id) == $id ? 'selected' : '' }}>{{ $entry }}</option>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('invited_mentees') ? 'is-invalid' : '' }}" name="invited_mentees[]" id="invited_mentees" multiple required>
                    ...
                    <option value="{{ $id }}" {{ (in_array($id, old('invited_mentees', $guestLecture->invitedMentees->pluck('id')->toArray()))) ? 'selected' : '' }}>{{ $invited_mentee }}</option>
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('guestsession_date_time') ? 'is-invalid' : '' }}" type="text" name="guestsession_date_time" id="guestsession_date_time" value="{{ old('guestsession_date_time', $guestLecture->guestsession_date_time) }}" required>
            ...
            <div class="form-group">
                ...
                @foreach(App\GuestLecture::GUEST_SESSION_DURATION_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('guest_session_duration') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="guest_session_duration_{{ $key }}" name="guest_session_duration" value="{{ $key }}" {{ old('guest_session_duration', $guestLecture->guest_session_duration) === (string) $key ? 'checked' : '' }} required>
