...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.ticketResponse.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.ticket-responses.update", [$ticketResponse->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('ticket_description') ? 'is-invalid' : '' }}" name="ticket_description_id" id="ticket_description_id" required>
                    @foreach($ticket_descriptions as $id => $entry)
                        <option value="{{ $id }}" {{ (old('ticket_description_id') ? old('ticket_description_id') : $ticketResponse->ticket_description->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('ticket_response') ? 'is-invalid' : '' }}" name="ticket_response" id="ticket_response">{!! old('ticket_response', $ticketResponse->ticket_response) !!}</textarea>
